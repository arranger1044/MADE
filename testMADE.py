...
class Tester(object):
    ...
    def _eval_valid_multiple_shuffle(self, model, nb_test_per_step):
        ...
        for stepIdx in range(len(steps)):
            ...
            for current_nb_shuffle in range(range_start, nb_test_per_step * step, step):
                print("/{0}\\".format(current_nb_shuffle), end=' ')
                # TODO Fix missing variable due to change of scope
                # avg_valid_nll, avg_valid_nll_std = 0, 0  # get_mean_error_and_std(model,
                # model.valid_log_prob, validset_theano.shape.eval()[0], shuffle_mask,
                # current_nb_shuffle)
                avg_valid_nll = 0
                nb_shuffles += [current_nb_shuffle]
                avg_nll += [round(avg_valid_nll, 2)]
            self._save_graph('{0}shuffle_test_temp'.format(
                step), x_label, y_label, (nb_shuffles, avg_nll, 'purple'))
            print(".", end=' ')
    ...
    def verify_reset_mask(self, in_size=10, hidden_sizes=[10, 3], nb_perm_mask=50):
        model = self._get_fake_model(in_size, hidden_sizes)
        print(in_size, hidden_sizes, nb_perm_mask, end=' ')
...
def verify_gradients(model, data):
    ...
    for h in range(len(model.parameters)):
        print('{0} grad diff : {1}'.format(model.parameters[h], np.mean(
            np.abs(parameters_gradient[h].ravel() - emp_grad_weights[h].ravel()))))
