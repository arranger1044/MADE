...
def batch_get_nearest_neighbours(samples, dataset):
    ...
    find_nearest_neighbour = theano.function(name="find_nearest_neighbour",
                                             inputs=[sample],
                                             outputs=data[
                                                 Te.argmin(Te.sum((data[:, None, :] - sample) ** 2, axis=2), axis=0)],
...
def get_nearest_neighbours(samples, dataset):
    ...
    find_nearest_neighbour = theano.function(name="find_nearest_neighbour",
                                             inputs=[sample],
                                             outputs=data[
                                                 Te.argmin(Te.sum((data - sample) ** 2, axis=1))],
                                             givens={data: dataset['train']['data']})
    neighbours = []
    for s in samples:
...
def save_samples(x, y, samples, dataset_name, model_name):
    total_final_array = []
    for w in range(y):
        total_final_array += [np.hstack([np.pad(zz.reshape((28, 28)), 1,
                                                mode='constant', constant_values=1) for zz in samples[w * x:w * x + x]])]

    plt.imsave('samples_{0}x{1}_{2}_{3}.png'.format(
        x, y, dataset_name, model_name), np.vstack(total_final_array), cmap=cm.Greys_r)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Sample from MADE trained on MNIST and generate an image of X by Y samples.')
    parser.add_argument(
        'model_path', help='Path to the experiment folder of the model to sample from.')
    parser.add_argument(
        'nb_samples_per_row', help="X: # of samples on each row.", type=int, default=10)
    parser.add_argument('nb_rows', help="Y: # of rows.", type=int, default=10)
    parser.add_argument('find_nearest_neighbour', metavar='find_nearest_neighbour', type=eval, choices=[
                        False, True], help="Generate the nearest neighbour (in trainset) image of the samples: {%(choices)s}", default=False)
    parser.add_argument('change_mask', metavar='change_mask',
                        help="Change mask for every row when generating samples: {%(choices)s}", type=eval, choices=[False, True])
