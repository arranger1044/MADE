...
class Dataset(object):
    ...
    def get_permutation(input_size):
        ...
        import itertools
        print("Generating dataset of all possible permutations for size ({0}) input_size ...".format(
            input_size), end=' ')
        dataset = []
        ...
        fake_dataset['valid'] = {
            'data': theano.shared(value=np.asarray(dataset, dtype=theano.config.floatX), borrow=True)}
        fake_dataset['train'] = {'data': theano.shared(
            value=np.asarray(dataset[:int(len(dataset) * 0.10)], dtype=theano.config.floatX), borrow=True)}
        fake_dataset['test'] = {
            'data': theano.shared(value=np.asarray(dataset, dtype=theano.config.floatX), borrow=True)}
    ...
    def get_fake(in_size=4, dataset_size=1):
        ...
        fake_dataset['valid'] = {'data': theano.shared(value=np.zeros(
            (dataset_size, in_size), dtype=theano.config.floatX), borrow=True), 'length': dataset_size}
        fake_dataset['train'] = {'data': theano.shared(value=np.zeros(
            (dataset_size, in_size), dtype=theano.config.floatX), borrow=True), 'length': dataset_size}
        fake_dataset['test'] = {'data': theano.shared(value=np.zeros(
            (dataset_size, in_size), dtype=theano.config.floatX), borrow=True), 'length': dataset_size}
        return fake_dataset
